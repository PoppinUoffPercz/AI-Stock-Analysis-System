...
def _write_records(path, worker, ready):
    ...
    for number in range(10):
        ...
        manifest = RunManifest.from_parts(stable={'engine': 'fixture'}, provenance={'run_id': run_id}, identity_hash=run_id)
...
def test_spawned_writers_preserve_records_and_deduplicate(tmp_path):
    ...
    workers = [context.Process(target=_write_records, args=(path, number, ready)) for number in range(4)]
    ...
    try:
        ...
        assert {r['run_id'] for r in records} == {'shared'} | {f'w{w}-{n}' for w in range(4) for n in range(1, 10)}
    finally:
        for worker in workers:
            if worker.is_alive():
                worker.terminate()
