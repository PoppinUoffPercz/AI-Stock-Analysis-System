...
def cmd_screener(args):
    ...
    from scion_omaha_bots.buffett_screener import BuffettScreener
    from scion_omaha_bots.notify import ScionNotifier
...
def cmd_analyze(args):
    ...
    from scion_omaha_bots.buffett_analyzer import BuffettAnalyzer
    from scion_omaha_bots.notify import ScionNotifier
...
def cmd_news(args):
    ...
    from scion_omaha_bots.buffett_news_engine import BuffettNewsEngine
    from scion_omaha_bots.notify import ScionNotifier
...
def cmd_premarket(args):
    ...
    from scion_omaha_bots.buffett_news_engine import BuffettNewsEngine
    from scion_omaha_bots.buffett_screener import BuffettScreener
    from scion_omaha_bots.earnings import format_earnings_brief, get_upcoming_earnings
    ...
    except Exception as e:  # noqa: BLE001 - yfinance market-data boundary
        print(f"  (Could not fetch market data: {e})")

    try:
        from scion_omaha_bots.credit_monitor import CreditMonitor
    ...
    except Exception:  # noqa: BLE001,S110 - optional pre-market credit enrichment
        pass
    ...
    alert_text = engine.generate_alert_text(results)
    if alert_text:
        significant = [line for line in alert_text.split("\n") if line.startswith("  [")]
        if significant:
    ...
    try:
        ...
        if brief:
            print("\n--- Upcoming Earnings ---")
            print(brief)
    except Exception:  # noqa: BLE001,S110 - optional earnings enrichment
        pass

    try:
        from scion_omaha_bots.ta_lib import compute_all as compute_ta
        print("\n--- Technical Pulse ---")
        for t in DEFAULT_WATCHLIST[:6]:
            ...
            except Exception:  # noqa: BLE001 - per-ticker provider/technical enrichment
                print(f"  {t:6s}:  N/A")
    except Exception:  # noqa: BLE001,S110 - optional technical enrichment
        pass

    try:
        from scion_omaha_bots.smart_money import get_smart_money_summary
        print("\n--- Smart Money Pulse ---")
        for t in DEFAULT_WATCHLIST[:4]:
            try:
                print(f"  {t:6s}:  {get_smart_money_summary(t)}")
            except Exception:  # noqa: BLE001 - per-ticker provider smart-money enrichment
                print(f"  {t:6s}:  N/A")
    except Exception:  # noqa: BLE001,S110 - optional smart-money enrichment
        pass
    ...
    legacy_report_dir = os.path.abspath(os.path.join(
        os.getcwd(), "..", "OneDrive", "Documents", "Obsidian Vault", "Stock Research", "Stock Analysis"
    ))
    report_dir = configured_outputs_root(legacy_report_dir)
    report_path = report_dir / f"{datetime.datetime.now().strftime('%Y-%m-%d')} Pre-Market Brief.md"
    try:
        ...
        with open(report_path, "a") as f:
            ...
            try:
                ...
                f.write(f"{cp}\n\n")
            except Exception:  # noqa: BLE001,S110 - optional report credit enrichment
                pass
            if not results.empty:
                fmt = _fmt_val
...
def cmd_combined(args):
    ...
    from scion_omaha_bots.buffett_portfolio import BuffettPortfolioManager
    from scion_omaha_bots.notify import ScionNotifier
    from scion_omaha_bots.portfolio_validation import load_portfolio

    print("\n" + "=" * 60)
    print("  COMBINED DUAL-AGENT PORTFOLIO VIEW")
    print("=" * 60)

    state_root = os.environ.get("STOCK_ANALYSIS_STATE_ROOT", os.getcwd())
    ...
    if os.path.exists(omaha_file):
        data = load_portfolio(omaha_file)
    ...
    if os.path.exists(scion_file):
        data = load_portfolio(scion_file)
    ...
    if scion_positions:
        print("\n  Scion-Bot Holdings:")
        spm = __import__("scion_omaha_bots.portfolio", fromlist=["ScionPortfolioManager"]).ScionPortfolioManager()
...
def cmd_run(args):
    ...
    from scion_omaha_bots.buffett_portfolio import BuffettPortfolioManager
    from scion_omaha_bots.buffett_screener import BuffettScreener
    from scion_omaha_bots.earnings import (
        format_earnings_brief,
        format_earnings_warning,
        get_upcoming_earnings,
    )
    from scion_omaha_bots.notify import ScionNotifier
    from scion_omaha_bots.performance_tracker import (
        log_run_cycle,
        log_screener_result,
        snapshot_portfolio,
    )
    ...
    if top_symbol and top_score >= 70:
        from scion_omaha_bots.earnings import get_earnings_analysis
    ...
    try:
        ...
        if warning:
            print(warning)
    except Exception:  # noqa: BLE001 - optional earnings-provider boundary
    ...
    for sym, pos in pm.positions.items():
        try:
            ...
            cp = float(cp)
        except Exception:  # noqa: BLE001 - yfinance price boundary; snapshot remains best-effort
            cp = 0
...
def build_parser():
    ...
    subparsers.add_parser("tracker", help="Show open positions from tracker")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "screener":
        ...
        from scion_omaha_bots.debate import cmd_compile, cmd_debate
        if args.compile:
