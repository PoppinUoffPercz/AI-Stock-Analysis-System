...
class CreditMonitor:
    ...
    def __init__(self, data_dir=None):
        self.data_dir = data_dir or os.environ.get(
            "STOCK_ANALYSIS_STATE_ROOT", os.getcwd()
        )
    ...
    def _load_state(self):
        ...
        try:
            ...
            raise ValueError(
                f"credit state {self.state_file}: invalid JSON at line {exc.lineno}"
            ) from exc
        if not isinstance(state, dict):
            raise ValueError(f"credit state {self.state_file}: expected a JSON object")
        return state
    ...
    def get_treasury_yields(self):
        ...
        if obb is not None:
            ...
            except Exception:  # noqa: BLE001,S110 - OpenBB provider fallback
                pass
        results = {}
        history = {}
        for tenor, ticker in TREASURY_TICKERS.items():
            ...
            except Exception as e:  # noqa: BLE001 - isolate one yfinance Treasury provider request
    ...
    def _get_etf_yield(self, ticker_str):
        ...
        except Exception:  # noqa: BLE001 - yfinance provider boundary
            return None
        return None
    ...
    def get_credit_spreads(self):
        ...
        except Exception as e:  # noqa: BLE001 - yfinance credit-spread provider boundary
            results["error"] = str(e)
    ...
    def get_sofr(self):
        ...
        try:
            ...
            if rate is not None:
                self.sofr = rate
                return rate
        except Exception:  # noqa: BLE001 - network provider fallback
            self.sofr = None
        self.sofr = None
        return None
    ...
    def scan_private_credit_news(self):
        ...
        for ticker in ["KKR", "ARCC", "FSK", "BX", "OBDC", "MAIN"]:
            ...
            except Exception:  # noqa: BLE001,S112 - isolate one provider ticker
                continue
    ...
    def save_report_to_vault(self, report_text):
        ...
        output_dir = configured_outputs_root(self.vault_base)
        try:
            output_dir.mkdir(parents=True, exist_ok=True)
        except OSError:
            return False
        date_str = datetime.datetime.now().strftime("%Y-%m-%d")
        path = output_dir / f"{date_str} Credit Report.md"
        try:
