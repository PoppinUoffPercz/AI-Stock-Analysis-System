...
from scion_omaha_bots.ta_lib import compute_atr, compute_macd, compute_rsi, compute_smas
...
DEBATE_DIR = os.environ.get("STOCK_ANALYSIS_STATE_ROOT", os.getcwd())
...
def fetch_debate_data(ticker):
    ...
    except Exception:  # noqa: BLE001 - yfinance price-history boundary
        hist = None
    ...
    except Exception:  # noqa: BLE001 - optional provider-derived context
        result["smart_money_score"] = None
        result["smart_money_label"] = None
    ...
    except Exception:  # noqa: BLE001 - yfinance news-provider boundary
        result["headlines"] = []
...
def save_debate_score(ticker, score):
    ...
    with process_lock(f"{DEBATE_SCORES_FILE}.lock"):
        scores = {}
        ...
        atomic_write_json(DEBATE_SCORES_FILE, scores)
...
def compile_report(ticker, wait=False):
    ...
    ticker = validate_identifier(ticker).upper()
