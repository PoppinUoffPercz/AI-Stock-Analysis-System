...
from scion_omaha_bots.portfolio_validation import (
    load_portfolio,
    normalize_symbol,
    positive_number,
)
from scion_omaha_bots.ta_lib import compute_atr, compute_rsi, compute_sma
...
class ScionPortfolioManager:
    ...
    def load_state(self):
        data = load_portfolio(self.portfolio_file)
        if data:
    ...
    def open_position(self, symbol, entry_price, stop_loss, target_1, target_2,
        ...
        positive_number(entry_price, 'entry_price')
        for field, value in (('stop_loss', stop_loss), ('target_1', target_1), ('target_2', target_2)):
    ...
    def check_position(self, symbol):
        ...
        try:
            ...
            if not hist.empty and len(hist) >= 20:
                ...
                atr_val = 0
                sma50 = 0
        except Exception:  # noqa: BLE001 - optional provider-derived technical context
            ta_rsi = {"value": 50, "regime": "neutral"}
            atr_val = 0
            sma50 = 0
    ...
    def scale_out(self, symbol, price, pct_to_sell=0.50, reason=""):
        ...
        positive_number(price, 'exit_price')
        positive_number(pct_to_sell, 'pct_to_sell', maximum=1)
        pos = self.positions[symbol]
