...
from scion_omaha_bots import entry_timing
from scion_omaha_bots.news_utils import extract_news_fields
from scion_omaha_bots.reflection import ReflectionLog
from scion_omaha_bots.smart_money import get_smart_money_score
from scion_omaha_bots.ta_lib import (
    compute_atr,
    compute_macd,
    compute_rsi,
    compute_smas,
...
def _discovery_tickers(limit=80, lists=("losers", "undervalued_large_caps")):
    ...
    except Exception:  # noqa: BLE001 - optional dependency may fail during import initialization
        return []
    symbols = []
    try:
        for name in lists:
            ...
            for s in df["symbol"].dropna().astype(str):
                ...
                if s.isalpha():
                    symbols.append(s)
    except Exception:  # noqa: BLE001,S110 - OpenBB discovery is optional
        pass
    seen = set()
    unique = []
    for s in symbols:
...
class ScionScreener:
    ...
    def fetch_fundamental_metrics(self, info, ticker_obj):
        ...
        if metrics["fcf"] is None:
            ...
            except Exception:  # noqa: BLE001,S110 - optional cash-flow fallback
                pass
    ...
    def run_screener(self):
        ...
        except Exception:  # noqa: BLE001,S110 - SPY timing context is optional
            pass
        ...
        for symbol in self.tickers:
            try:
                ...
                earnings_days = None
                try:
                    from scion_omaha_bots.earnings import get_upcoming_earnings
                    er = get_upcoming_earnings([symbol], max_days=45)
                    if er:
                        earnings_days = er[0]["days_away"]
                except Exception:  # noqa: BLE001,S110 - earnings context is optional
                    pass
                ...
                except Exception:  # noqa: BLE001,S110 - smart-money context is optional
                    pass
            ...
            except Exception as e:  # noqa: BLE001 - isolate one provider-backed ticker scan
    ...
    def _write_shadow_log(self):
        ...
        if not self.shadow_rows:
            ...
            return 0
        output_root = configured_outputs_root(os.getcwd())
        output_root.mkdir(parents=True, exist_ok=True)
        path = str(output_root / "shadow_log.csv")
...
if __name__ == "__main__":
    ...
    if results_df.empty:
        ...
        output_root = configured_outputs_root(os.getcwd())
        output_root.mkdir(parents=True, exist_ok=True)
        output_path = output_root / "screener_output.md"
