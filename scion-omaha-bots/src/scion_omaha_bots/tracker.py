...
def _ensure_file(path, headers):
    if not os.path.exists(path):
        try:
            ...
            with process_lock(f"{path}.lock"):
                if os.path.exists(path):
                    return
                with open(path, "w", newline="", encoding="utf-8") as f:
                    csv.writer(f).writerow(headers)
        except OSError as exc:
            print(f"  [tracker] Could not initialize {path}: {exc}")
...
def _get_sector(ticker):
    ...
    except Exception:  # noqa: BLE001 - yfinance company-info boundary
        return "Unknown"
...
def _get_day_range(ticker, date_str):
    ...
    Matches the requested trading day by its YYYY-MM-DD date string.
    """
    try:
    ...
    except Exception:  # noqa: BLE001 - yfinance OHLC-history boundary
        return None
...
class Tracker:
    ...
    def load_open_positions(self):
        if os.path.exists(self.positions_file):
            try:
                ...
                raise ValueError(
                    f"open positions {self.positions_file}: invalid JSON at line {exc.lineno}"
                ) from exc
            if not isinstance(positions, dict):
                raise ValueError(f"open positions {self.positions_file}: expected a JSON object")
            return positions
        return {}

    def save_open_positions(self, positions):
        atomic_write_json(self.positions_file, positions)
    ...
    def log_exit(self, ticker, exit_price=None, exit_reason="manual"):
        ...
        except Exception as e:  # noqa: BLE001 - reflection logging must not block trade-state logging
